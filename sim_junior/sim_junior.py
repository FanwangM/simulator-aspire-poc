...
SVV_1 = JuniorVial(
    identifier="SV VIAL 1",
    contained_by=JUNIOR_LAB["SVV SLOT 1"].identifier,
    chemical_content={"solid amine": 1000},
    vial_type="SV",
)
SVV_2 = JuniorVial(
    identifier="SV VIAL 2",
    contained_by=JUNIOR_LAB["SVV SLOT 2"].identifier,
...
def needle_dispense(
    ...
    ins2 = JuniorInstruction(
        device=Z1_ARM,
        action_name="concurrent_aspirate",
        action_parameters={
    ...
    ins4 = JuniorInstruction(
        device=Z1_ARM,
        action_name="concurrent_dispense",
        action_parameters={
...
ins_list5 = solid_dispense(
    sv_vial=SVV_1,
    sv_vial_slot=JUNIOR_LAB["SVV SLOT 1"],
...
ins_list7 = needle_dispense(
    DCM_VIALS, JUNIOR_LAB["SLOT OFF-1"], MRV_VIALS, JUNIOR_LAB["SLOT 2-3-1"], 10
)
ins_list7[0].preceding_instructions.append(ins_list6[-1].identifier)

ins_list8 = needle_dispense(
    DCM_VIALS,
    JUNIOR_LAB["SLOT OFF-1"],
    RSO2Cl_STOCK_SOLUTION_VIALS,
    JUNIOR_LAB["SLOT 2-3-2"],
    10,
)
ins_list8[0].preceding_instructions.append(ins_list7[-1].identifier)

ins_list9 = pdp_dispense(
    PYRIDINE_VIAL,
    JUNIOR_LAB["SLOT 2-3-2"],
    RACK_D_TIPS,
    JUNIOR_LAB["SLOT 2-3-3"],
    MRV_VIALS,
...
ins_stir1 = JuniorInstruction(
    device=JUNIOR_LAB["SLOT 2-3-1"],
    action_name="wait",
    action_parameters={"wait_time": 300},
    description="wait for 5 min",
)
ins_stir2 = JuniorInstruction(
    device=JUNIOR_LAB["SLOT 2-3-2"],
    action_name="wait",
    action_parameters={"wait_time": 300},
    description="wait for 5 min",
)
ins_stir3 = JuniorInstruction(
    device=JUNIOR_LAB["SLOT 2-3-3"],
    action_name="wait",
    action_parameters={"wait_time": 300},
    description="wait for 5 min",
)
...
ins_list10[0].preceding_instructions = [
    ins_stir1.identifier,
    ins_stir2.identifier,
    ins_stir3.identifier,
]

ins_stir21 = JuniorInstruction(
    device=JUNIOR_LAB["SLOT 2-3-1"],
    action_name="wait",
    action_parameters={"wait_time": 7200},
...
model = Model(
    env, JUNIOR_LAB, wdir=os.path.abspath("./"), model_name=f"con-{CONCURRENCY}"
)
